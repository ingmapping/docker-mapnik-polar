...
#
# Render tiles in an polar projection: EPSG 3575 (https://nsidc.org/data/atlas/epsg_3575.html)
# OriginX:-13000000 / OriginY: 13000000 / Resolution: 101562.5 / BBOX: -13000000, -13000000, 13000000, 13000000.
#
...
def main():
    ...
    if options.onlyinteresting:
        ...
        sql = """
        SELECT 'point' AS type, osm_id, name, ST_X(way), ST_Y(way), ST_X(ST_Transform(way, 3575)), ST_Y(ST_Transform(way, 3573)) FROM ant_point
        ...
        SELECT 'line' AS type, osm_id, name, ST_X(ST_Centroid(way)), ST_Y(ST_Centroid(way)), ST_X(ST_Transform(ST_Centroid(way), 3575)), ST_Y(ST_Transform(ST_Centroid(way), 3575)) FROM ant_line
            WHERE (place IS NOT NULL AND place IN ('hamlet', 'town', 'isolated_dwelling', 'cape', 'locality', 'island', 'islet'))
            OR building IS NOT NULL
            OR aeroway IS NOT NULL
        ...
        SELECT 'polygon' AS type, osm_id, name, ST_X(ST_Centroid(way)), ST_Y(ST_Centroid(way)), ST_X(ST_Transform(ST_Centroid(way), 3575)), ST_Y(ST_Transform(ST_Centroid(way), 3575)) FROM ant_polygon
