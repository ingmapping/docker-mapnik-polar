...
#
# Render polar antarctica tiles in an polar projection: EPSG 3412 (https://nsidc.org/data/atlas/epsg_3412.html)
...
def main():
    ...
    if options.onlyinteresting:
        ...
        sql = """
        SELECT 'point' AS type, osm_id, name, ST_X(way), ST_Y(way), ST_X(ST_Transform(way, 3412)), ST_Y(ST_Transform(way, 3412)) FROM ant_point
        ...
        SELECT 'line' AS type, osm_id, name, ST_X(ST_Centroid(way)), ST_Y(ST_Centroid(way)), ST_X(ST_Transform(ST_Centroid(way), 3412)), ST_Y(ST_Transform(ST_Centroid(way), 3412)) FROM ant_line
            WHERE (place IS NOT NULL AND place IN ('hamlet', 'town', 'isolated_dwelling', 'cape', 'locality', 'island', 'islet'))
            OR building IS NOT NULL
            OR aeroway IS NOT NULL
        ...
        SELECT 'polygon' AS type, osm_id, name, ST_X(ST_Centroid(way)), ST_Y(ST_Centroid(way)), ST_X(ST_Transform(ST_Centroid(way), 3412)), ST_Y(ST_Transform(ST_Centroid(way), 3412)) FROM ant_polygon
